...
//    }
    ...
    /**
     This function is used for left swipe the view.
     - Author:  Nitesh Garg
     - Returns: Void
     */
    func leftSwipe(_ closure: @escaping () -> ()) {
        let sleeve = ClosureSleeve(closure)
        let recognizer = UISwipeGestureRecognizer(target: sleeve, action: #selector(ClosureSleeve.invoke))
    ...
    /**
     This function is used for right swipe the view.
     - Author:  Nitesh Garg
     - Returns: Void
     */
    func rightSwipe(_ closure: @escaping () -> ()) {
        let sleeve = ClosureSleeve(closure)
        let recognizer = UISwipeGestureRecognizer(target: sleeve, action: #selector(ClosureSleeve.invoke))
